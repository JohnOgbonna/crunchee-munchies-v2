...
const SelectedVariationDisplay: React.FC<SelectedVariationDisplayProps> = ({
  ...
  const existingOrder =
    orders[selectedVariation.parentId]?.variations[selectedVariation.id]
      ?.quantity || 0;
  ...
  const handleQuantityChange = (newQuantity: number, viaButton = false) => {
    ...
    if (viaButton && newQuantity < minOrderQuantity) {
      toast.error(
        `Minimum order quantity for ${selectedVariation.name} is ${minOrderQuantity}`
      );
      return;
    }
    if (viaButton && newQuantity > maxOrderQuantity) {
      toast.error(
        `Maximum order quantity for ${selectedVariation.name} is ${maxOrderQuantity}`
      );
      return;
    }
  ...
  const handleAddToOrder = () => {
    ...
    addOrder(selectedVariation.parentId, {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 text-center md:text-left md:items-center md:w-[70%] md:min-w-[530px] lg:min-w-[600px] mb-6">
      <motion.div
        ...
        className="h-[400px] md:min-w-[200px] lg:min-w-[350px] flex items-center justify-center overflow-hidden"
      >
        <Image
          src={selectedVariation.url as string}
      ...
      <div>
        <div className="flex flex-col items-center">
          {selectedVariation.savings && (
            <p className="text-white font-semibold py-2 px-4 bg-red-700 rounded-[20px] mb-4 md:hidden">
          ...
          <button
            className={`mt-4 px-6 py-2 font-bold rounded-lg ${orderUpdated

...
const OrderDetailView = ({ orderId, email }: OrderDetailViewProps) => {
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 md:p-6 bg-[#fff8e1] text-slate-700 rounded-lg shadow-md">
            ...
            <div className="text-gray-800 space-y-1 mb-6">
                <p>
                    <span className="font-semibold">Status:</span> {orderData.order.status}
                </p>
                <p>
            ...
            <div className="space-y-6">
                {orderData.orderItems.map((item) => (
                    <div
                        key={item.id}
                        className="flex items-center justify-between gap-4 border-b pb-4 border-gray-200 max-w-[500px]"
                    >
                        <img
                        ...
                        <div className="flex flex-col gap-y-1">
                            ...
                            <p className="text-sm md:text-[1rem] hover:underline hover:text-blue-500">
                               <Link href={`/shop/${item.parent.id}?`}> Item Type: <span>{item.parent.name}</span></Link>
                            </p>
                        ...
                        <button className="p-2 bg-orange-300 text-white font-bold sm:text-md rounded-md hover:underline">
                            <Link href={`/shop/${item.parent.id}?variant=${item.variant.id}`} target="_blank">View Item</Link>
                        </button>
                    </div>
                ))}
            </div>

            <div className="mt-8 pt-4 border-t border-gray-200 space-y-2 max-w-[500px]">
                <p>

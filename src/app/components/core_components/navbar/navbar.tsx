...
import { navSections, adminNavSections, NavSubsection, NavSubsections } from "@/app/data/structures";
...
function NavbarComponent() {
    ...
    const navSubsections = useMemo(() => {
        const updatedSubsections: NavSubsections = { shop: {} };
        if (!items) return updatedSubsections;
    ...
    const sections = isAdminPath ? adminNavSections : navSections;
    ...
    return (
        ...
        >
            <div className="md:flex md:justify-between max-w-[1440px] mx-auto">
                <div className="flex justify-between bg-orange-200 p-2 max-w-[1400px]">
                    <Link href={isAdminPath ? "/admin" : "/"}>
                        <h1 className="font-bold cursor-pointer hover:underline text-[1.2rem] hover:scale-105 transition-all ease-in-out">
                            {isAdminPath ? "Admin" : "Crunchee Munchies"}
                        </h1>
                    </Link>
                ...
                `}>
                    <li onClick={() => setIsOpen(false)} className="absolute top-2 right-2 cursor-pointer md:hidden">
                    ...
                    {Object.entries(sections).map(([key, value], index) => (
                        ...
                        >
                            <div className="flex items-center md:w-auto" onClick={() => setIsOpen(false)}>
                                <Link href={`/${isAdminPath ? `admin/${key}` : key}`} className="text-[1.3rem] md:text-[1rem] hover:underline hover:scale-105 transition-all ease-in-out">
                                    {value}
                                </Link>
                                {!isAdminPath && navSubsections?.[key] && (
                                    <svg
            ...
            {!isAdminPath && (
                <OrderPanel isCartOpen={isCartOpen} setIsCartOpen={setIsCartOpen} items={items} />
            )}
        </motion.nav>
    );
}
